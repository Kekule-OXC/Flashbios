...
int DriveSecurityChange(unsigned uIoBase, int driveId, ide_command_t ide_cmd, char *password) {
	...
	if(BootIdeIssueAtaCommand(uIoBase, ide_cmd, &tsicp1)) {
		printk ("Error issuing command\n");
		return 1;
	}
	...
	if (BootIdeWaitNotBusy(uIoBase))	
	{
		//printk("Error waiting for drive\n");
		return 1;
	}
	...
	if(BootIdeIssueAtaCommand(uIoBase, IDE_CMD_IDENTIFY, &tsicp)) 
	{
		printk("Error identifying drive\n");
		return 1;
	}
	BootIdeWaitDataReady(uIoBase);
	if(BootIdeReadData(uIoBase, baBuffer, IDE_SECTOR_SIZE)) 
	{
		printk("Error reading ident-data");
		return 1;
	}

	if (ide_cmd == IDE_CMD_SECURITY_DISABLE && drive_info[128]&0x0002)
		return 1;

	if (ide_cmd == IDE_CMD_SECURITY_SET_PASSWORD && !(drive_info[128]&0x0002))
		return 1;

	if (ide_cmd == IDE_CMD_SECURITY_UNLOCK && drive_info[128]&0x0004)
		return 1;
