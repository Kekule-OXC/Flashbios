...
void BootVgaInitializationKernelNG(CURRENT_VIDEO_MODE_DETAILS * pcurrentvideomodedetails) {
	...
	if (video_encoder==ENCODER_XCALIBUR) pcurrentvideomodedetails->m_nVideoModeIndex = VIDEO_MODE_640x480;
   ...
   MMIO_H_OUT32(riva.PRAMDAC,0,0x884,0x0);
	...
	usedVideoMode = pcurrentvideomodedetails->m_nVideoModeIndex;
	...
	/* All other cable types - normal SDTV */
		switch(pcurrentvideomodedetails->m_nVideoModeIndex) {
			...
			// shift all modes
			pcurrentvideomodedetails->m_dwMarginXInPixelsRecommended=60;
			pcurrentvideomodedetails->m_dwMarginYInLinesRecommended=40;					
